...
export function Clock(props: allWidType) {
  const [nowTime, setNowTime] = useState(new Date(2000, 0, 1, 0, 0, 0));
    ...
    <WidBasic theme={props.theme}>
      <div id={styles.clockChild}>
        <div id={styles.dateArea}>
          <p id={styles.clDate}>{`${String(
            nowTime.getFullYear()
          )}年 (令和${String(nowTime.getFullYear() - 2018)}年)`}</p>
          <p>{`${String(nowTime.getMonth() + 1)}月${String(
            nowTime.getDate()
          )}日(${
        ...
        <div id={styles.timeArea}>
          <p id={styles.clTime}>{`${String(nowTime.getHours()).padStart(
            2,
            "0"
          )}:${String(nowTime.getMinutes()).padStart(2, "0")}`}</p>
        </div>
      </div>
    </WidBasic>
  );
...
export function Weather(props: weatherWidType) {
  ...
  useEffect(() => {
    const fetchwj = async () => {
      const returnJson: any = await fetch("/api/gwe?city=" + props.point).then(
        (res) => res.json()
      );
  ...
  return (
    <WidBasic
      title={`${weatherJson.location}の天気`}
      theme={props.theme}
      topSide={
        <a
          onClick={() => {
    ...
    >
      <OneWeather
        date={`${today.getMonth()}月${today.getDate()}日`}
        weatherJson={weatherJson[1]}
      ></OneWeather>
      <OneWeather
        date={`${tommorow.getMonth()}月${tommorow.getDate()}日`}
        weatherJson={weatherJson[2]}
      ></OneWeather>
    </WidBasic>
  );
}
...
export function Trend(props: allWidType) {
  ...
  return (
    <WidBasic title="トレンド" theme={props.theme}>
      {trendArr.map((onePhrase) => (
        <a
          key={`trend_${onePhrase}`}
          href={`https://www.google.com/search?q=${onePhrase}`}
          rel="nofollow noopener noreferrer"
          target="_blank"
          className={styles.trendA}
        >
          {onePhrase}
        </a>
      ))}
...
export function News(props: newsType) {
  ...
  return (
    <WidBasic
      title={allNews[nowNewsNum].title}
      theme={props.theme}
      topSide={topSideButton}
    >
